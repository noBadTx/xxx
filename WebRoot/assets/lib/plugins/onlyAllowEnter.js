     ...
     return function(jquery){
		(function($){	 
			 ...
			 $.fn.onlyAllowEnter.defaults = {
			   ...
			   */
		     function allowEnterNumberZero2One(event){
			 ...
			 function onlyNumber(keyCode){
				 if(isBackspace(keyCode) && ((keyCode > 57 && keyCode< 96) || keyCode < 48 || keyCode>105 ) && keyCode!=110){
				     return false;
				 }
				 return true;
			 };
			  ...
			  *   为指定的文本框绑定onKeyDown事件
			  */
			 function allowEnterFloat(event){
			    var keyCode = event.keyCode;
				var _val = $(this).val();
				if(keyCode == 190){
			       if(_val == ''){
			 ...
			 */
			function allowEnterNumber(event){
			    var keyCode = event.keyCode;
				 if(!onlyNumber(keyCode)){
				    return false;
				 }
